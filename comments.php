<?php
	 ...
	 * that contains both the current comments and the comment
	 * form.
	 *
	...
	/*
	* If the current post is protected by a password and the
	* visitor has not yet entered the password we will
		...
		<h2>
			<?php
				comments_number(
					...
					_n('% Comment', '% Comments', get_comments_number(), 'bonestheme')
				);
			?>
		</h2>
		<?php if (previous_comments_link('&lt; Older Comments') || next_comments_link('Newer Comments &gt;')) { ?>
			<nav class="pagination comment-menu">
				<ul>
